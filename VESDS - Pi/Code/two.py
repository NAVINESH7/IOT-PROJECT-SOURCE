...
BAUD_RATE = 115200
PWRKEY_PIN = 18
PHONE_NUMBER = '+918122735200'  # 🔸 Replace with target phone number
...
def init_gps(ser):
    ...
    for attempt in range(3):
        ...
        if not mode_ok and '+CME ERROR: 58' in response:
            print("GPSMODE not supported — skipping")
...
def send_sms(ser, phone, message):
    ...
    time.sleep(1)
    ser.write((message + chr(26)).encode())  # CTRL+Z
    time.sleep(5)
    response = ser.read_all().decode('utf-8', errors='ignore')
    if 'OK' in response:
...
try:
    ...
    location_msg = f"{SMS_MESSAGE_PREFIX}{google_maps_link}"
